...
nicheStep.enter((ctx) => {
  ctx.reply(
    ...
    '📝 **Шаг 1 из 3**\n\n' +
    'Опишите вашу нишу максимально конкретно (1–3 предложения).\n\n' +
...
keywordsStep.enter((ctx) => {
  ctx.reply(
    ...
    'Пример: "английский язык, IT, релокация, работа за границей, программисты"',
    { parse_mode: 'Markdown' }
  );
});
...
countryStep.action('confirm', async (ctx) => {
  ...
  if (saved) {
    await ctx.reply(
      ...
      'Сейчас я подберу для вас первый инфоповод. Это может занять до минуты...',
      { parse_mode: 'Markdown' }
    );
    ...
    await ctx.reply(
      '✅ Готово! С завтрашнего дня вы будете получать один инфоповод каждый день.\n\n' +
